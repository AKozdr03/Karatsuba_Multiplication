...
(
    ...
    // Sygnał CE: Potok pracuje TYLKO wtedy, gdy nadajnik daje dane (tvalid) 
    // i odbiornik (DMA) jest gotowy je przyjąć (tready)
    ...
    karatsuba_pipeline karatsuba_pipeline_inst( 
        .clk(s00_axis_aclk),                                          
        .rst_n(rst),                                                  
        .ce(pipe_ce),              // <--- Podpięty Clock Enable                         
...
module karatsuba_pipeline #(
    ...
    input  wire                 ce,         // <--- Port Clock Enable
    input  wire                 valid_in,              
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            v_stg1 <= 0;
        end else if (ce) begin      // <--- Zamrożenie potoku gdy brak danych
            z0_stg1    <= x_l * y_l;
            z2_stg1    <= x_h * y_h;
